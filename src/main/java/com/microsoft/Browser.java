...
/**
 * A Browser is created via {@link BrowserType#launch BrowserType.launch()}. An example of using a
...
public interface Browser extends AutoCloseable {
  ...
  class NewContextOptions {
    ...
    /**
     * When using {@link Page#navigate Page.navigate()}, {@link Page#route
     * Page.route()}, {@link Page#waitForURL Page.waitForURL()}, {@link
     * Page#waitForRequest Page.waitForRequest()}, or {@link
     * Page#waitForResponse Page.waitForResponse()} it takes the base URL in consideration by using
     ...
     * feature, supported values are {@code "light"} and {@code "dark"}. See {@link Page#emulateMedia
     ...
     * Page#emulateMedia Page.emulateMedia()} for more details. Passing {@code null} resets emulation
     ...
     * BrowserContext#grantPermissions BrowserContext.grantPermissions()} for more details. Defaults
     * to none.
     */
     ...
     * BrowserContext#close BrowserContext.close()} for the HAR to be saved.
     */
     ...
     * See {@link Page#emulateMedia Page.emulateMedia()} for more details. Passing {@code null} resets
    ...
    /**
     * Populates context with given storage state. This option can be used to initialize context with logged-in information
     * obtained via {@link BrowserContext#storageState BrowserContext.storageState()}. Path to the
  ...
  /**
   * In case this browser is obtained using {@link BrowserType#launch BrowserType.launch()}, closes
   ...
   * {@link BrowserContext#close BrowserContext.close()} on any {@code BrowserContext} instances you
   * explicitly created earlier using {@link Browser#newContext Browser.newContext()} **before**
   * calling {@link Browser#close Browser.close()}.
   *
   ...
   * context via {@link BrowserContext#close BrowserContext.close()} when your code is done with the
   * {@code BrowserContext}, and before calling {@link Browser#close Browser.close()}. This will
   ...
   * testing frameworks should explicitly create {@link Browser#newContext Browser.newContext()}
   * followed by the {@link BrowserContext#newPage BrowserContext.newPage()} to control their exact
   * life times.
   *
   * @since v1.8
   */
   ...
   *
   * <p> You can use {@link Browser#startTracing Browser.startTracing()} and {@link
   * Browser#stopTracing Browser.stopTracing()} to create a trace file that can be opened in Chrome
