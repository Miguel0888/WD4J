 ...
 * with {@link Page#routeWebSocket Page.routeWebSocket()} or {@link
 * BrowserContext#routeWebSocket BrowserContext.routeWebSocket()}, the {@code WebSocketRoute}
 ...
 * }</pre>
 *
 * <p> Since we do not call {@link WebSocketRoute#connectToServer WebSocketRoute.connectToServer()}
 ...
 * Calling {@link WebSocketRoute#connectToServer WebSocketRoute.connectToServer()} returns a
 ...
 *
 * <p> However, if you call {@link WebSocketRoute#onMessage WebSocketRoute.onMessage()} on the
 * original route, messages from the page to the server **will not be forwarded** anymore, but should instead be handled by
 * the {@code handler}.
 *
 * <p> Similarly, calling {@link WebSocketRoute#onMessage WebSocketRoute.onMessage()} on the
 ...
 * WebSocketRoute#onMessage WebSocketRoute.onMessage()} in both directions, there is no automatic
 * forwarding at all.
 * <pre>{@code
 * page.routeWebSocket("/ws", ws -> {
 *   WebSocketRoute server = ws.connectToServer();
...
public interface WebSocketRoute {
   ...
   * WebSocketRoute#onMessage WebSocketRoute.onMessage()} is called on the server-side {@code
   * WebSocketRoute}.</li>
   ...
   * WebSocketRoute#onMessage WebSocketRoute.onMessage()} is called on the original {@code
   * WebSocketRoute}.</li>
   * </ul>
   *
   ...
   * when {@link WebSocketRoute#onClose WebSocketRoute.onClose()} handler is set up, the default
   ...
   * messages by responding to them with {@link WebSocketRoute#send WebSocketRoute.send()},
   * forwarding them to the server-side connection returned by {@link WebSocketRoute#connectToServer
   ...
   * manually by calling {@link WebSocketRoute#send WebSocketRoute.send()}. See examples at the top
   * for more details.
   *
   ...
   * the result of {@link WebSocketRoute#connectToServer WebSocketRoute.connectToServer()}, sends
