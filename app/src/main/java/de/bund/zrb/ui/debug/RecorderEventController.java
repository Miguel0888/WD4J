...
import de.bund.zrb.ui.debug.WDUiAppender;
...
public final class RecorderEventController {
    ...
    /** optionaler Filter: nur Events für diese UserContext-ID durchlassen */
    private volatile String userContextFilter;

    /** Ziel-UI (kann ein RecorderTabUi, EventMonitorWindow-Adapter etc. sein) */
    private final RecorderTabUi ui;

    /** Zugehörige Session (u.a. für raw-Event-Protokollierung und Flags) */
    ...
    // optionale Zeitdifferenz-Anzeige (Δt seit letztem Event)
    ...
    private final BiConsumer<String, Object> sink = new BiConsumer<String, Object>() {
        @Override public void accept(String eventName, Object payload) {
            ...
            // 3) in die UI hängen -> IMMER auf den EDT!
            SwingUtilities.invokeLater(() -> {
                try {
                    ui.appendEvent(eventName, label);
                } catch (Throwable t) {
                    // letzter Rettungsanker, damit eine einzelne Exception die Pipeline nicht killt
                    System.err.println("[RecorderEventController] appendEvent failed: " + t);
                }
            });
        }
    };
    ...
    public synchronized void start(BrowserContext context) {
        ...
        // System.out.println("[RecorderEventController] started (context), flags=" + session.getEventFlags());
    }
    ...
    public synchronized void start(Page page) {
        ...
        active.add(a);
        // System.out.println("[RecorderEventController] started (page), flags=" + session.getEventFlags());
    }
    ...
    private JLabel buildEventLabel(String name, Object payload) {
        ...
        lbl.putClientProperty("eventName", name);
