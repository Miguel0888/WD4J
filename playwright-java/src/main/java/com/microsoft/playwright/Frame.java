...
/**
 * At every point of time, page exposes its current frame tree via the {@link Page#mainFrame
 * Page.mainFrame()} and {@link Frame#childFrames Frame.childFrames()} methods.
 *
 * <p> {@code Frame} object's lifecycle is controlled by three events, dispatched on the page object:
 * <ul>
 * <li> {@link Page#onFrameAttached Page.onFrameAttached()} - fired when the frame gets attached to the
 * page. A Frame can be attached to the page only once.</li>
 * <li> {@link Page#onFrameNavigated Page.onFrameNavigated()} - fired when the frame commits navigation
 * to a different URL.</li>
 * <li> {@link Page#onFrameDetached Page.onFrameDetached()} - fired when the frame gets detached from
...
public interface Frame {
  ...
  class IsHiddenOptions {
    ...
    public Boolean strict;
    /**
     * @deprecated This option is ignored. {@link Frame#isHidden Frame.isHidden()} does not wait for the element
  ...
  class IsVisibleOptions {
    ...
    public Boolean strict;
    /**
     * @deprecated This option is ignored. {@link Frame#isVisible Frame.isVisible()} does not wait for the element
   ...
   * Frame#evalOnSelector Frame.evalOnSelector()} would wait for the promise to resolve and return
   * its value.
   *
   * <p> <strong>Usage</strong>
   * <pre>{@code
   ...
   * <p> If {@code expression} returns a <a
   * href='https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise'>Promise</a>, then {@link
   * Frame#evalOnSelectorAll Frame.evalOnSelectorAll()} would wait for the promise to resolve and
   * return its value.
   *
   * <p> <strong>Usage</strong>
   * <pre>{@code
   ...
   *
   * <p> If the function passed to the {@link Frame#evaluate Frame.evaluate()} returns a
   * non-[Serializable] value, then {@link Frame#evaluate Frame.evaluate()} returns {@code
   ...
   * }</pre>
   *
   * <p> {@code ElementHandle} instances can be passed as an argument to the {@link Frame#evaluate
   * Frame.evaluate()}:
   * <pre>{@code
   ...
   *
   * <p> The only difference between {@link Frame#evaluate Frame.evaluate()} and {@link
   * Frame#evaluateHandle Frame.evaluateHandle()} is that {@link
   * Frame#evaluateHandle Frame.evaluateHandle()} returns {@code JSHandle}.
   *
   * <p> If the function, passed to the {@link Frame#evaluateHandle Frame.evaluateHandle()}, returns a
   * <a href='https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise'>Promise</a>, then
   * {@link Frame#evaluateHandle Frame.evaluateHandle()} would wait for the promise to resolve and
   * return its value.
   *
   * <p> <strong>Usage</strong>
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p> {@code JSHandle} instances can be passed as an argument to the {@link Frame#evaluateHandle
   ...
   *
   * <p> This is an inverse of {@link ElementHandle#contentFrame ElementHandle.contentFrame()}. Note
   ...
   * <p> <strong>Usage</strong>
   *
   * <p> The {@link Frame#waitForFunction Frame.waitForFunction()} can be used to observe viewport size
   * change:
   * <pre>{@code
   * import com.microsoft.playwright.*;
   *
