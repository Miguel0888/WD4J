...
public interface Page extends AutoCloseable {
   ...
   * beforeunload}. Listener **must** either {@link Dialog#accept Dialog.accept()} or {@link
   * Dialog#dismiss Dialog.dismiss()} the dialog - otherwise the page will <a
   ...
   * }</pre>
   *
   * <p> <strong>NOTE:</strong> When no {@link Page#onDialog Page.onDialog()} or {@link
   * BrowserContext#onDialog BrowserContext.onDialog()} listeners are present, all dialogs are
   ...
   * respond to it via setting the input files using {@link FileChooser#setFiles
   ...
   * BrowserContext#onPage BrowserContext.onPage()}, but only for popups relevant to this page.
   *
   ...
   * this network request, use {@link BrowserContext#route BrowserContext.route()} and {@link
   * BrowserContext#onRequest BrowserContext.onRequest()} respectively instead of similar methods on
   * the {@code Page}.
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p> <strong>NOTE:</strong> Use {@link Page#waitForLoadState Page.waitForLoadState()} to wait until the page gets to a
   ...
   * {@link Page#route Page.route()} or {@link BrowserContext#route
   ...
   * with {@link Page#onRequestFinished Page.onRequestFinished()} event and not with {@link
   * Page#onRequestFailed Page.onRequestFailed()}. A request will only be considered failed when the
  ...
  class CheckOptions {
     ...
     * value can be changed by using the {@link BrowserContext#setDefaultTimeout
     * BrowserContext.setDefaultTimeout()} or {@link Page#setDefaultTimeout Page.setDefaultTimeout()}
     * methods.
     */
  ...
  class GoBackOptions {
     ...
     * be changed by using the {@link BrowserContext#setDefaultNavigationTimeout
     * BrowserContext.setDefaultNavigationTimeout()}, {@link BrowserContext#setDefaultTimeout
     * BrowserContext.setDefaultTimeout()}, {@link Page#setDefaultNavigationTimeout
     * Page.setDefaultNavigationTimeout()} or {@link Page#setDefaultTimeout Page.setDefaultTimeout()}
     * methods.
     */
    public Double timeout;
    /**
  ...
  class IsHiddenOptions {
    ...
    public Boolean strict;
    /**
     * @deprecated This option is ignored. {@link Page#isHidden Page.isHidden()} does not wait for the element to
  ...
  class IsVisibleOptions {
    ...
    public Boolean strict;
    /**
     * @deprecated This option is ignored. {@link Page#isVisible Page.isVisible()} does not wait for the element
  ...
  class WaitForCloseOptions {
     ...
     * default value can be changed by using the {@link BrowserContext#setDefaultTimeout
  ...
  class WaitForResponseOptions {
    /**
     * Maximum wait time in milliseconds, defaults to 30 seconds, pass {@code 0} to disable the timeout. The default value can
     * be changed by using the {@link BrowserContext#setDefaultTimeout
     * BrowserContext.setDefaultTimeout()} or {@link Page#setDefaultTimeout Page.setDefaultTimeout()}
     * methods.
     */
    public Double timeout;
   ...
   * BrowserContext.addInitScript()} and {@link Page#addInitScript Page.addInitScript()} is not
   * defined.
   *
   ...
   * unless {@code force} option is set. If the element is detached during the checks, the whole action is retried.</li>
   * <li> Scroll the element into view if needed.</li>
   * <li> Use {@link Page#mouse Page.mouse()} to click in the center of the element, or the specified
   * {@code position}.</li>
   ...
   * unless {@code force} option is set. If the element is detached during the checks, the whole action is retried.</li>
   * <li> Scroll the element into view if needed.</li>
   * <li> Use {@link Page#mouse Page.mouse()} to double click in the center of the element, or the
   ...
   * Page#evalOnSelector Page.evalOnSelector()} would wait for the promise to resolve and return its
   * value.
   *
   * <p> <strong>Usage</strong>
   * <pre>{@code
   ...
   * <p> If {@code expression} returns a <a
   * href='https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise'>Promise</a>, then {@link
   * Page#evalOnSelectorAll Page.evalOnSelectorAll()} would wait for the promise to resolve and
   * return its value.
   *
   * <p> <strong>Usage</strong>
   * <pre>{@code
   ...
   *
   * <p> If the function passed to the {@link Page#evaluate Page.evaluate()} returns a
   * non-[Serializable] value, then {@link Page#evaluate Page.evaluate()} resolves to {@code
   ...
   * }</pre>
   *
   * <p> {@code ElementHandle} instances can be passed as an argument to the {@link Page#evaluate
   * Page.evaluate()}:
   * <pre>{@code
   ...
   *
   * <p> The only difference between {@link Page#evaluate Page.evaluate()} and {@link
   * Page#evaluateHandle Page.evaluateHandle()} is that {@link
   * Page#evaluateHandle Page.evaluateHandle()} returns {@code JSHandle}.
   *
   * <p> If the function passed to the {@link Page#evaluateHandle Page.evaluateHandle()} returns a <a
   * href='https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise'>Promise</a>, then {@link
   * Page#evaluateHandle Page.evaluateHandle()} would wait for the promise to resolve and return its
   * value.
   *
   * <p> <strong>Usage</strong>
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p> {@code JSHandle} instances can be passed as an argument to the {@link Page#evaluateHandle
   * Page.evaluateHandle()}:
   * <pre>{@code
   ...
   *
   * <p> See {@link BrowserContext#exposeBinding BrowserContext.exposeBinding()} for the context-wide
   * version.
   *
   ...
   * awaited.
   *
   * <p> See {@link BrowserContext#exposeFunction BrowserContext.exposeFunction()} for context-wide
   * exposed function.
   *
   ...
   * instead.
   *
   * <p> To send fine-grained keyboard events, use {@link Locator#pressSequentially
   ...
   * Selectors#setTestIdAttribute Selectors.setTestIdAttribute()} to configure a different test id
   ...
   *
   * <p> See also {@link Locator#filter Locator.filter()} that allows to match by another criteria, like
   ...
   * unless {@code force} option is set. If the element is detached during the checks, the whole action is retried.</li>
   * <li> Scroll the element into view if needed.</li>
   * <li> Use {@link Page#mouse Page.mouse()} to hover over the center of the element, or the specified
   * {@code position}.</li>
   * </ol>
   *
   * <p> When all steps combined have not finished during the specified {@code timeout}, this method throws a {@code
   ...
   * }</pre>
   *
   * @param handler Receives the {@code Dialog} object, it **must** either {@link Dialog#accept Dialog.accept()} or
   * {@link Dialog#dismiss Dialog.dismiss()} the dialog - otherwise the page will <a
   * href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/EventLoop#never_blocking">freeze</a> waiting for the
   * dialog, and actions like click will never finish.
   * @since v1.10
   */
   ...
   * your normal test flow, instead of using {@link Page#addLocatorHandler
   ...
   * mouse state being unchanged.For example, consider a test that calls {@link Locator#focus Locator.focus()} followed by
   * {@link Keyboard#press Keyboard.press()}. If your handler clicks a button between these two
   * actions, the focused element most likely will be wrong, and key press will happen on the unexpected element. Use {@link
   * Locator#press Locator.press()} instead to avoid this problem.Another example is a series of mouse actions, where {@link Mouse#move Mouse.move()} is followed
   * by {@link Mouse#down Mouse.down()}. Again, when the handler runs between these two actions, the
   * mouse position will be wrong during the mouse down. Prefer self-contained actions like {@link
   * Locator#click Locator.click()} that do not rely on the state being unchanged by a handler.
   *
   * <p> <strong>Usage</strong>
   *
  ...
  /**
   * Removes all locator handlers added by {@link Page#addLocatorHandler Page.addLocatorHandler()}
   * for a specific locator.
   *
   * @param locator Locator passed to {@link Page#addLocatorHandler Page.addLocatorHandler()}.
   * @since v1.44
   */
   ...
   * BrowserContext#request BrowserContext.request()} on the page's context. See {@link
   * BrowserContext#request BrowserContext.request()} for more details.
   *
   * @since v1.16
   */
   ...
   *
   * <p> <strong>NOTE:</strong> {@link Page#route Page.route()} will not intercept requests intercepted by Service Worker. See
   ...
   *
   * <p> <strong>NOTE:</strong> {@link Page#route Page.route()} will not intercept the first request of a popup page. Use
   * {@link BrowserContext#route BrowserContext.route()} instead.
   *
   * <p> <strong>Usage</strong>
   *
   ...
   * BrowserContext#route BrowserContext.route()}) when request matches both handlers.
   *
   ...
   * </ul>
   *
   * <p> <strong>NOTE:</strong> {@link Page#setDefaultNavigationTimeout Page.setDefaultNavigationTimeout()} takes priority over
   * {@link Page#setDefaultTimeout Page.setDefaultTimeout()}, {@link
   * BrowserContext#setDefaultTimeout BrowserContext.setDefaultTimeout()} and {@link
   * BrowserContext#setDefaultNavigationTimeout BrowserContext.setDefaultNavigationTimeout()}.
   *
   ...
   *
   * <p> <strong>NOTE:</strong> {@link Page#setDefaultNavigationTimeout Page.setDefaultNavigationTimeout()} takes priority over
   * {@link Page#setDefaultTimeout Page.setDefaultTimeout()}.
   *
   ...
   *
   * <p> <strong>NOTE:</strong> {@link Page#setExtraHTTPHeaders Page.setExtraHTTPHeaders()} does not guarantee the order of
   ...
   * Browser#newContext Browser.newContext()} allows to set viewport size (and more) for all pages
   * in the context at once.
   *
   * <p> {@link Page#setViewportSize Page.setViewportSize()} will resize the page. A lot of websites
   * don't expect phones to change size, so you should set the viewport size before navigating to the page. {@link
   * Page#setViewportSize Page.setViewportSize()} will also reset {@code screen} size, use {@link
   * Browser#newContext Browser.newContext()} with {@code screen} and {@code viewport} parameters if
   ...
   * unless {@code force} option is set. If the element is detached during the checks, the whole action is retried.</li>
   * <li> Scroll the element into view if needed.</li>
   * <li> Use {@link Page#touchscreen Page.touchscreen()} to tap the center of the element, or the
   * specified {@code position}.</li>
   * </ol>
   *
   * <p> When all steps combined have not finished during the specified {@code timeout}, this method throws a {@code
  ...
  /**
   * Removes all routes created with {@link Page#route Page.route()} and {@link
   * Page#routeFromHAR Page.routeFromHAR()}.
   *
   * @since v1.41
   */
  void unrouteAll();
  /**
   * Removes a route created with {@link Page#route Page.route()}. When {@code handler} is not
   ...
   * truthy value. Will throw an error if the page is closed before the {@link Page#onConsoleMessage
   ...
   * <p> <strong>Usage</strong>
   *
   * <p> The {@link Page#waitForFunction Page.waitForFunction()} can be used to observe viewport size
   * change:
   * <pre>{@code
   * import com.microsoft.playwright.*;
   *
   ...
   * }</pre>
   *
   * <p> To pass an argument to the predicate of {@link Page#waitForFunction Page.waitForFunction()}
   * function:
   * <pre>{@code
   * String selector = ".foo";
   ...
   * an error if the page is closed before the {@link Page#onRequestFinished
