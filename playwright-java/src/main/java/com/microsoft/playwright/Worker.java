...
public interface Worker {
   ...
   *
   * <p> If the function passed to the {@link Worker#evaluate Worker.evaluate()} returns a
   * non-[Serializable] value, then {@link Worker#evaluate Worker.evaluate()} returns {@code
   ...
   *
   * <p> The only difference between {@link Worker#evaluate Worker.evaluate()} and {@link
   * Worker#evaluateHandle Worker.evaluateHandle()} is that {@link
   * Worker#evaluateHandle Worker.evaluateHandle()} returns {@code JSHandle}.
   *
   * <p> If the function passed to the {@link Worker#evaluateHandle Worker.evaluateHandle()} returns a
   * <a href='https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise'>Promise</a>, then
   * {@link Worker#evaluateHandle Worker.evaluateHandle()} would wait for the promise to resolve and
   * return its value.
   *
   * @param expression JavaScript expression to be evaluated in the browser context. If the expression evaluates to a function, the function is
   * automatically invoked.
